...
**Notebook:** [`notebooks/pipeline.ipynb`](notebooks/pipeline.ipynb) - the single consolidated
notebook, runs end to end on Google Colab. It contains the three required parts:
...
**Deployment option (ML):** the trained artifact `guarantorlens_serving.joblib` (regenerated by the
notebook into the local, git-ignored `models/` folder) is served by the backend's `POST /assess-risk`
endpoint and exercised through **Swagger UI** (see Backend below).


### Run the notebook (Colab)
1. Open `notebooks/pipeline.ipynb` in Google Colab.
...
The training pipeline lives in [`notebooks/pipeline.ipynb`](notebooks/pipeline.ipynb), not a separate
...
**Feature-set comparison (held-out, committed to `report_assets/result_tables/00_final_metrics.csv`):**
...
tuning tables in
`report_assets/result_tables/` alongside the leaderboard, confusion matrices, SHAP summary, calibration
curve, and ROC/PR curves in `report_assets/figures/`.

> **Consolidated notebook.** The full pipeline (main model + other models + all analyses) now lives in
> one explained notebook, [`notebooks/pipeline.ipynb`](notebooks/pipeline.ipynb).

### Robustness and fairness checks

`pipeline.ipynb` has a **"Robustness and fairness checks"** section (eight cells) that stress-tests the
deployed model and exports one CSV/figure each (committed under `report_assets/result_tables/` and
`report_assets/figures/`):

- **Out-of-time test** (train on the earliest loans, test on the latest): `06_out_of_time.csv`
- **95% confidence intervals** for PR-AUC, ROC-AUC, precision, recall (bootstrap): `06_bootstrap_ci.csv`
- **Calibration quality** (Brier score, expected calibration error, bin counts): `06_calibration_metrics.csv`
- **Loan-officer review burden** at each threshold: `06_review_burden.csv`
...
- **Fairness across the interest-rate (public/private employment) proxy**: `06_fairness_subgroup.csv`
- **Cold-start** (new borrowers, guarantors with no track record): `06_cold_start.csv`

### Report documentation

Report-ready write-ups (facts and tables to write the prose from) are provided as separate deliverable
documents (`.md` and `.docx`): `ROBUSTNESS_FAIRNESS`, `TRACEABILITY_MATRIX` (requirements-to-tests plus
code coverage), and `CH4_TESTING_ADDITIONS` (traceability, audit-log verification, fallback testing).
The committed evidence they draw from (figures and result tables) is in `report_assets/`.
...
Committed (tracked) layout:

```
guarantorLens_mission_capstone_ML/
├── notebooks/pipeline.ipynb # the consolidated pipeline: viz + engineering + models +
│                            #   robustness/fairness + all analyses (Colab)
├── report_assets/           # committed evidence the report draws from:
│   ├── figures/             #   leaderboard, confusion, SHAP, calibration, ROC/PR, 06_* robustness
│   ├── result_tables/       #   metrics CSVs incl. 00_final_metrics and the 06_* robustness tables
│   ├── diagrams/            #   pipeline, architecture, ERD, class, use-case, etc.
│   └── screenshots/         #   captioned app screenshots
├── assets/screenshots/      # app screenshots
├── LICENSE
└── README.md
```

Local only, **git-ignored** (regenerated or kept off GitHub): `models/` (notebook outputs and the
`serving.joblib` artifact), `docs/` (report write-ups delivered separately), and `data/` (real SACCO
records, never pushed).
